...
QString CMakeListsBuild::buildCtxApp(QJsonObject json)
{
  ...
  QString subPrjStr;
  QString library = "${PROJECT_NAME}";
  QStringList components;
  QStringList subPrjs;
  QJsonArray jsonDirs = json["dirs"].toArray();
  QJsonArray jsonComponents = json["Qt"].toArray();

  for (QJsonValue value: jsonComponents)
    {
      components.append(value.toString());
    }
...
//    }
  if (jsonComponents.size() > 0)
    {
      ...
      for (int i = 0; i < jsonComponents.size(); i++)
        {
          packages += " " + jsonComponents[i].toString();
          library += " Qt5::" + jsonComponents[i].toString();
        }
      packages += " REQUIRED)\n";
    }
  ...
  for (QJsonValue value: jsonDirs)
    {
      subPrjStr += "add_subdirectory(" + value.toString() + ")\n";
      library += " " + value.toString();
    }


  QString ctx = header.arg(project, packages + subPrjStr, sources, library);
...
QString CMakeListsBuild::buildCtxLib(QJsonObject json)
{
  ...
  for (QJsonValue value: jsonDirs)
    {
      packages += "add_subdirectory(" + value.toString() + ")\n";
      library += " " + value.toString();
    }
